...
async def create_posts(item: PostCreate, db: AsyncSession = Depends(get_db),
                 current_user: DBUser = Depends(get_current_user)):
    current_user_json = jsonable_encoder(current_user)
    current_user_json = current_user_json.get('id')
    result = await create_post(db=db, item=item, user_id=current_user_json)
    return result


@router.get('/posts_list_for_current_user', response_model=List[PostList])
async def get_all_posts_for_current_user(db: AsyncSession = Depends(get_db),
                                   current_user: DBUser = Depends(get_current_user)):
    current_user_json = jsonable_encoder(current_user)
    get_current_user_json = current_user_json.get('id')
    result = await get_current_user_posts(db=db, user_id=get_current_user_json)
    return result


@router.get("/post/", response_model=List[PostList])
async def get_post_list(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    result = await get_posts(db=db, skip=skip, limit=limit)
    return result


@router.get("/post/{post_id}", response_model=PostSingle)
async def get_post_detail(post_id: int, db: AsyncSession = Depends(get_db)):
    result = await get_post_single(db=db, post_id=post_id)
    if not result:
        raise HTTPException(
            status_code=400,
            detail="Not Found",
